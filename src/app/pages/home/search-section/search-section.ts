...
@Component({
  ...
  standalone: true,
  imports: [ReactiveFormsModule, ZardInputDirective, ZardButtonComponent, ZardPopoverComponent, ZardPopoverDirective],
  template: `
      ...
      <div class="container relative">
        <form [formGroup]="searchForm" class="flex items-center gap-2 md:gap-4">
          ...
          <ng-template #popoverContent>
            <z-popover class="w-[90vw] sm:w-[92vw] lg:w-[900px] absolute -right-[80px] sm:-right-[170px] md:-right-[265px] top-2">
              <div class="space-y-2">
                <h4 class="font-medium leading-none">Address</h4>
                <div class="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-3">
                  <input z-input placeholder="Country" formControlName="country" class="px-3 md:px-4 h-10" />
                  <input z-input placeholder="Region" formControlName="region" class="px-3 md:px-4 h-10" />
                  <input z-input placeholder="City" formControlName="city" class="px-3 md:px-4 h-10" />
                  <input z-input placeholder="Zip code" formControlName="zip" class="px-3 md:px-4 h-10" />
                </div>
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-3">
                  <input z-input placeholder="Min price" formControlName="minPrice" class="px-3 md:px-4 h-10" />
                  <input z-input placeholder="Max price" formControlName="maxPrice" class="px-4 h-10" />
                </div>

                <div class="flex items-center justify-end gap-3 mt-5 md:mt-7">
                  <button z-button zType="destructive" class="flex-1 md:w-30" (click)="resetParams()">Clear</button>
...
export class SearchSection {
  ...
  private cleanParams(values: any) {
    // remove empty fields from query params
